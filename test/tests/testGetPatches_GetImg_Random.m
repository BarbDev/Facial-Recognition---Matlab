...
%%%% RANDOM TEST %%%%
nbrPatchesRand = 6;
patches = zeros(pSize*pSize,nbrPatchesRand, 'uint8');

RandomV = getRandom(nbrPatchesRand, pSize, RWidth, RHeight);
for i = 1:nbrPatchesRand
    x = RandomV(1,i);
    y = RandomV(2,i);
    patch = imcrop(R, [x y pSize-1 pSize-1]);
    subplot(2,3,i)
    imshow(patch)
    patches(:,i) = reshape(patch, [pSize*pSize 1]);
end
figure
imshow(patches)

%%% Try to get patch place
