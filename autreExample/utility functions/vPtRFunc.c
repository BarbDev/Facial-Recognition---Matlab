...
void mexFunction(
    ...
    if (nargin != 7 || nargout != 1)
        mexErrMsgTxt("Usage: X = vPtRFunc (Y,n1,n2,N1,N2,rowSt,colSt)");
    ...
    n = n1*n2;
    pargout [0] = mxCreateDoubleMatrix(n, q, mxREAL);
    X = mxGetPr( pargout [0]);
    ...
    if (rowSt > 1){
        ...
        if (sgn2 > 0){
            ...
            for (w2=n2-res2; w2<n2; w2++)
                for (w1=0; w1<rowSt-1; w1++){
                row = w2*n1+w1;
                X[col*n+row] = Y[(col*n2+w2-n2+res2)*N1+w1];
                }
        }
    }
    ...
    if (colSt > 1){
        ...
        if (sgn1 > 0){
            ...
            for (w2=0; w2<colSt-1; w2++)
                ...
                row = w2*n1+w1;
                X[col*n+row] = Y[w2*N1+col*n1+w1-n1+res1];
                }
        }
    }
    ...
    for (j=0; j<colN; j++){
        for(i=0; i<rowN; i++){
            ...
            for(w2=0;w2<n2;w2++)
                for(w1=0;w1<n1;w1++){
                row = w2*n1+w1;
                X[col*n+row] = Y[(j*n2+w2+colSt-1)*N1+i*n1+w1+rowSt-1];
                }
        }
        if (sgn1 > 0){
            col++;
            for (w2=0; w2<n2; w2++)
                for (w1=n1-res1; w1<n1; w1++){
                row = w2*n1+w1;
                X[col*n+row] = Y[(j*n2+w2+colSt-1)*N1+rowN*n1+w1-n1+res1+rowSt-1];
                }
        }
    }
    if (sgn2 > 0){
        for(i=0; i<rowN; i++){
            ...
            for(w2=n2-res2; w2<n2; w2++)
                for(w1=0; w1<n1; w1++){
                row = w2*n1+w1;
                X[col*n+row] = Y[(colN*n2+w2-n2+res2+colSt-1)*N1+i*n1+w1+rowSt-1];
                }
        }
        if (sgn1 > 0){
            col++;
            for(w2=n2-res2; w2<n2; w2++)
                for (w1 = n1-res1; w1<n1; w1++){
                row = w2*n1+w1;
                X[col*n+row] = Y[(colN*n2+w2-n2+res2+colSt-1)*N1+i*n1+w1-n1+res1+rowSt-1];
                }
        }
    }
}